...
import {
  ...
  DialogContent, DialogActions, Tooltip, TextField, InputAdornment,
  AppBar, Toolbar
} from '@mui/material';
    ...
    <Box sx={{ minHeight: '100vh', backgroundColor: '#f8f4f5' }}>
      ...
      <Container maxWidth="xl" sx={{ py: 5 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 4, flexWrap: 'wrap', gap: 2 }}>
          <Box>
            <Button
              ...
              sx={{ color: '#888', fontFamily: '"Lato", sans-serif', textTransform: 'none', mb: 1, pl: 0 }}
            >
              Back
            </Button>
            <Typography sx={{ fontFamily: '"Cormorant Garamond", serif', fontSize: '2rem', fontWeight: 700 }}>
        ...
        {loading ? (
          ...
          <Paper elevation={0} sx={{ border: '1px solid rgba(0,0,0,0.08)', borderRadius: 3, overflow: 'hidden' }}>
            <TableContainer>
              <Table>
                <TableHead>
                  <TableRow sx={{ backgroundColor: '#faf7f7' }}>
                    {['Shop Name', 'Category', 'District', 'Phone', 'Rating', 'Featured', t('admin.actions')].map((h) => (
                      <TableCell
                        key={h}
                        sx={{ fontFamily: '"Lato", sans-serif', fontWeight: 700, fontSize: '0.8rem', color: '#555', textTransform: 'uppercase', letterSpacing: 0.5 }}
                      >
                        {h}
                      </TableCell>
                    ))}
                ...
                <TableBody>
                  {filtered.map((vendor) => (
                    <TableRow key={vendor._id} hover sx={{ '&:hover': { backgroundColor: '#fdf5f6' } }}>
                      <TableCell sx={{ fontFamily: '"Lato", sans-serif', fontWeight: 600 }}>{vendor.shop_name}</TableCell>
                      <TableCell>
                        <Chip
                          label={vendor.category}
                          size="small"
                          sx={{ backgroundColor: 'rgba(122,28,46,0.08)', color: '#7a1c2e', fontFamily: '"Lato", sans-serif', fontSize: '0.72rem' }}
                        />
                      </TableCell>
                      <TableCell sx={{ fontFamily: '"Lato", sans-serif', color: '#555' }}>{vendor.district}</TableCell>
                      <TableCell sx={{ fontFamily: '"Lato", sans-serif', color: '#555' }}>{vendor.phone}</TableCell>
                      <TableCell sx={{ fontFamily: '"Lato", sans-serif' }}>{vendor.rating || '—'}</TableCell>
                      <TableCell>
                        <Chip
                          label={vendor.featured ? 'Yes' : 'No'}
                          size="small"
                          sx={{
                            backgroundColor: vendor.featured ? 'rgba(5,150,105,0.1)' : 'rgba(0,0,0,0.06)',
                  ...
                  {filtered.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={7} align="center" sx={{ py: 6, fontFamily: '"Lato", sans-serif', color: '#888' }}>
                        {t('common.no_results')}
                      </TableCell>
                    </TableRow>
                  )}
      ...
      <Dialog open={!!deleteId} onClose={() => setDeleteId(null)} PaperProps={{ sx: { borderRadius: 3 } }}>
        <DialogTitle sx={{ fontFamily: '"Cormorant Garamond", serif', fontSize: '1.4rem' }}>
          Confirm Delete
        </DialogTitle>
        <DialogContent>
        ...
        <DialogActions sx={{ p: 2 }}>
          <Button onClick={() => setDeleteId(null)} sx={{ fontFamily: '"Lato", sans-serif', textTransform: 'none', color: '#555' }}>
            {t('admin.cancel')}
          </Button>
          <Button
            ...
            disabled={deleting}
            variant="contained"
            sx={{ backgroundColor: '#dc2626', fontFamily: '"Lato", sans-serif', textTransform: 'none', borderRadius: 2, '&:hover': { backgroundColor: '#b91c1c' } }}
          >
            {deleting ? <CircularProgress size={18} color="inherit" /> : t('admin.delete_vendor')}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
